...
namespace Motiviti.Enkidu
{
	...
	//	bool itemUsageEnabled = true;
		...
		public void SetVisible(bool b)
		{
			...
			//foreach (Transform el in uiElements)
			//	el.gameObject.SendMessage ("ResetButton", SendMessageOptions.DontRequireReceiver);
			//SaveState();
		}

		void Update()
		{
			//Debug.Log (Time.time + " scrollIndex: " + scrollPosition);
		}
		...
		public void Remove(InventoryItem item)
		{
			...
			if(items.Contains(item)){
				...
				{
					foreach(InventoryItem i in items){  //reposition all lower then removed
			...
			if(holding)
			{
				...
				if(col.Length > 0)
				{
					foreach(Collider2D c in col)
					{
						...
						{
							//Vector3 localHitpoint=itemHolder.InverseTransformPoint(worldPosition);  //get localhitpoint in inventory
	...
	tadej
			if (enableArrowLight)
			{
				arrow.sprite = arrowClickedSprite;
				if (arrowLight)arrowLight.enabled = true;
			}
			yield return new WaitForSeconds(0.1f);
			if (enableArrowLight)
			{
				arrow.sprite = arrowUnclickedSprite;
				if (arrowLight) arrowLight.enabled = false;
			}
			if (isHidden){
